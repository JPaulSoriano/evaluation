...
<div class="col-sm-4">
    <div class="form-group">
        ...
        <form action="{{ route('faculties.reportCategory', ['faculty' => $faculty]) }}" method="GET">
    ...
    <button class="btn btn-sm btn-primary mb-2" type="button" onclick="
        printJS({
                printable: 'category',
                type: 'html',
                header: '<div style=\'display: flex; align-items: center;\'><img src=\'{{ asset('images/logo.png') }}\' style=\'max-width: 100px; margin-right: 10px;\'><h3>{{ $faculty->full_name }} Report per Category for A.Y. {{ $currentAcademicYear }} {{ Request::input('quarter') ? ' - Quarter ' . Request::input('quarter') : ' - All Quarters' }}</h3></div>'
            })
    ">Print</button>
</div>
<form id="category">
        ...
        <tbody>
            @foreach ($categoryAverages as $category)
                <tr>
                    ...
                    <td>{{ number_format($category['average_rate'], 2) }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
