...
<div class="col-sm-12">
    ...
    <table class="table table-bordered" id="printJS-form">
        <thead>
            <tr>
                <th colspan="2">{{ $faculty->full_name }} Report per Section for A.Y. {{ $currentAcademicYear }} {{ Request::input('quarter') ? ' - Quarter ' . Request::input('quarter') : ' - All Quarters' }}</th>
            </tr>
        </thead>
        <thead>
            <tr>
        ...
        <tbody>
            @foreach ($sectionAverages as $sectionAverage)
                <tr>
                    ...
                    <td>{{ $sectionAverage['average_rate'] }}</td>
                </tr>
            @endforeach
        </tbody>
    </table>
